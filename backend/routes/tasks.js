...
router.post('/:id/start-rewrite', requireAuth, async (req, res) => {
  try {
    ...
    if (rows[0].status !== 'extracted' && rows[0].status !== 'done') return res.status(400).json({ code: 400, msg: '文案尚未提取完成' });
    ...
    if (editedTranscript && editedTranscript.trim()) {
      ...
      await db.query(
        "UPDATE tasks SET status='pending', stage='extracted', progress=50, thinking='', result=$2, updated_at=NOW() WHERE id=$1",
        [req.params.id, JSON.stringify(existing)]
      );
    } else {
      await db.query(
        "UPDATE tasks SET status='pending', stage='extracted', progress=50, thinking='', updated_at=NOW() WHERE id=$1",
        [req.params.id]
      );
    }
