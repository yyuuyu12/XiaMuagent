...
时长：${durationLabel} · 风格：${styleLabel} · 平台：${platformLabel}${meta.angle ? `\n用户初始观点：${meta.angle}` : ''}`;
  ...
  if (stage === 'direction') {
    return `你是顶级短视频选题策划。本阶段产出【选题方向】供用户挑选或确定，不要写正文文案。
${common}
## 本阶段任务（看用户这句话的意图，二选一）
- 用户在**要方向/要候选/还没定**（如"帮我想3个方向""换个角度""再来几个"）→ 给出 **3 个差异化的完整候选方向**，每个都要详细到用户能直接比较、一眼选出。三个方向要真的不同（切入角度/人群/情绪各异），不要换汤不换药。
- 用户**已明确选定**某个（如"用第2个""就定XX这个方向""第1个再聚焦痛点"）→ 只输出那 1 条，细化得更扎实。

## 每个方向必须写全这几项（这是"详细"的标准，不许省）
- 角度：从什么切入、为什么这个切口新
- 目标人群：具体到一类人 + 他此刻的痛点/欲望
- 核心钩子：开头能让人停下来的那一下（要具体到能直接用的话术感觉，不是"用反差"这种抽象）
- 一句话选题：可直接当标题/开场的那句
- 为什么能爆：这个方向踩中了什么情绪或需求

## 输出格式
开头一句话点出这几个方向的差异（≤40字），然后用标签包裹（多个候选都放在同一对标签内）：
【选题方向】
方向一：〔给个短名〕
· 角度：…
· 目标人群：…
